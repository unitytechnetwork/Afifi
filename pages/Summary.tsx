...
const Summary: React.FC = () => {
  ...
  const DEFECT_TERMS = ['Defective', 'Fault', 'Faulty', 'Damaged', 'Failed', 'Low', 'Broken', 'Leaking', 'Corroded', 'Loose', 'Blocked', 'Blown', 'Expired', 'Missing', 'Abnormal'];
        ...
        try {
          ...
          if (sys.label === 'Main Fire Alarm') {
            ...
            systems.forEach((s: any) => {
              s.integrationItems?.forEach((i: any) => { if (i.status === 'Faulty' || DEFECT_TERMS.includes(i.status)) faults.push({ system: sys.label, section: s.zoneName, itemLabel: i.label, description: i.remarks || 'Integration failure.', photo: i.photo, path: sys.path, status: 'Open' }); });
              const gasDevices = [{ label: 'Smoke Detectors', count: parseInt(s.smokeQty || 0) }, { label: 'Heat Detectors', count: parseInt(s.heatQty || 0) }, { label: 'Alarm Bells', count: parseInt(s.bellQty || 0) }, { label: 'Flashers', count: parseInt(s.flashingLightQty || 0) }].filter(d => d.count > 0);
            ...
            if (units.length > 0) sysInventory.push({ location: data.location || 'Pump Room', items: units });
            
            ['jockeyUnit', 'dutyUnit', 'standbyUnit'].forEach(uKey => {
              const u = data[uKey];
              if (u && (u.status === 'Fault' || DEFECT_TERMS.includes(u.status))) {
                faults.push({ system: sys.label, section: 'Mechanical', itemLabel: u.label, description: u.remarks || 'Pump performance failure.', photo: u.photo, path: sys.path, status: 'Open' });
              }
            });

            if (data.panelLampsStatus === 'Fault') faults.push({ system: sys.label, section: 'Panel', itemLabel: 'Indicator Lamps', description: data.panelLampsRemarks || 'Faulty indicator detected.', photo: data.panelLampsPhoto, path: sys.path, status: 'Open' });
            if (data.panelWiringStatus === 'Fault') faults.push({ system: sys.label, section: 'Panel', itemLabel: 'Internal Wiring', description: data.panelWiringRemarks || 'Wiring logic error.', photo: data.panelWiringPhoto, path: sys.path, status: 'Open' });
            if (data.selectorSwitchStatus === 'Fault') faults.push({ system: sys.label, section: 'Panel', itemLabel: 'Selector Switch', description: data.selectorSwitchRemarks || 'Switch mechanical failure.', photo: data.selectorSwitchPhoto, path: sys.path, status: 'Open' });
          }
          else {
            ...
            items.forEach((item: any, idx: number) => {
              ...
              const hasFault = Object.entries(item).some(([k, v]) => !['id','photos','remarks'].includes(k) && typeof v === 'string' && DEFECT_TERMS.includes(v));
              if (hasFault) {
                let faultDesc = 'Defect detected.';
                if (typeof item.remarks === 'string' && item.remarks) faultDesc = item.remarks;
                else if (typeof item.remarks === 'object' && item.remarks) {
                  const vals = Object.values(item.remarks).filter(v => typeof v === 'string' && v !== '');
                  if (vals.length > 0) faultDesc = vals.join('. ');
                }
                let faultPhoto = item.photo;
                if (!faultPhoto && typeof item.photos === 'object' && item.photos) faultPhoto = Object.values(item.photos).find(p => typeof p === 'string' && p.startsWith('data:')) as string;
                faults.push({ system: sys.label, section: 'Inventory', itemLabel: item.serial || item.location || `Unit #${idx+1}`, description: faultDesc, photo: faultPhoto, path: sys.path, status: 'Open' });
              }
            });
  ...
  const handleFinalize = () => {
    if (!sigData) return;
    if (faultRegistry.length > 0) sendLocalNotification("Critical System Deficiencies Recorded", `Technical Dossier #${auditId} has been committed with ${faultRegistry.length} faults.`);
  ...
  return (
    <div className="flex flex-col h-full bg-background-dark overflow-y-auto no-scrollbar pb-32">
      ...
      {isSubmitting ? (
        <div className="flex-1 flex flex-col items-center justify-center p-10 bg-[#181111] fixed inset-0 z-[100]"><div className="w-20 h-20 border-4 border-primary/20 border-t-primary rounded-full animate-spin mb-8" /><h2 className="text-xl font-black italic uppercase tracking-widest text-white">Generating Dossier</h2></div>
      ) : (
        <div className="p-4 flex flex-col gap-6">
          <div className="bg-surface-dark p-6 rounded-3xl border-l-8 border-primary shadow-2xl relative overflow-hidden">
             ...
             <div className="flex flex-col gap-1 relative z-10">
                ...
                <div className="flex gap-4 mt-3">
                   <div className="flex flex-col"><span className="text-[8px] font-black text-text-muted uppercase">Inspected Systems</span><span className="text-lg font-black text-white">{systemStats.totalSystems}</span></div>
                   <div className="flex flex-col"><span className="text-[8px] font-black text-text-muted uppercase">Verified Assets</span><span className="text-lg font-black text-emerald-500">{systemStats.totalAssets}</span></div>
                </div>
             </div>
          </div>

          <div className="flex flex-col gap-4">
            <h3 className="text-[10px] font-black text-text-muted uppercase tracking-[0.3em] italic px-2">System Census & Inventory</h3>
            <div className="flex flex-col gap-3">
              {(Object.entries(inventoryRegistry) as [string, LocationSummary[]][]).map(([sysLabel, locations]) => (
                <div key={sysLabel} className="bg-surface-dark rounded-3xl border border-white/5 overflow-hidden shadow-xl">
                   <div className="bg-white/5 px-5 py-3 border-b border-white/5 flex items-center justify-between"><span className="text-[10px] font-black uppercase tracking-widest text-emerald-500 italic">{sysLabel}</span><span className="text-[8px] font-black text-text-muted uppercase tracking-tighter">Inventory Audit</span></div>
                   <div className="p-2 flex flex-col gap-2">
                      {locations.map((loc, lIdx) => (
                        <div key={lIdx} className="bg-background-dark/40 rounded-2xl p-4 border border-white/5">
                           ...
                           <div className="grid grid-cols-2 gap-2">{loc.items.map((item, iIdx) => (<div key={iIdx} className="bg-white/5 p-2 rounded-xl flex items-center justify-between border border-white/5"><span className="text-[9px] font-bold text-text-muted truncate mr-2">{item.label}</span><span className="text-[11px] font-black text-white bg-primary/20 px-2 py-0.5 rounded shadow-inner">{item.count}</span></div>))}</div>
                        </div>
                      ))}
                   </div>
                </div>
          ...
          <div className="flex flex-col gap-4">
             <div className="flex items-center justify-between px-2 mt-4">
                ...
                <span className={`text-[9px] font-black px-3 py-1 rounded-full uppercase tracking-widest ${faultRegistry.length > 0 ? 'bg-primary text-white animate-pulse' : 'bg-emerald-500/20 text-emerald-500'}`}>{faultRegistry.length} Faults Found</span>
             </div>
             {faultRegistry.length > 0 ? (Object.entries(faultsBySystem) as [string, FaultRecord[]][]).map(([system, faults]) => (
                <div key={system} className="bg-surface-dark rounded-3xl border border-white/5 overflow-hidden shadow-xl">
                   <div className="bg-primary/5 px-5 py-3 border-b border-white/5 flex items-center justify-between"><span className="text-[10px] font-black uppercase tracking-widest text-primary italic">{system}</span><span className="text-[8px] font-black text-text-muted uppercase">Defects Detected</span></div>
                   <div className="flex flex-col divide-y divide-white/5">
                      {faults.map((fault, idx) => (
                         <div key={idx} onClick={() => navigate(fault.path)} className="p-5 flex gap-4 active:bg-white/5 transition-colors cursor-pointer group">
                            <div className="w-16 h-16 rounded-xl bg-background-dark border border-primary/20 shrink-0 overflow-hidden shadow-inner flex items-center justify-center">{fault.photo ? <img src={fault.photo} className="w-full h-full object-cover" /> : <span className="material-symbols-outlined text-primary/40">no_photography</span>}</div>
                            <div className="flex-1 min-w-0">
                               <div className="flex justify-between items-start mb-1"><span className="text-[7px] font-black text-text-muted uppercase tracking-widest">{fault.section}</span><span className="text-[7px] font-black text-primary uppercase bg-primary/10 px-1.5 py-0.5 rounded tracking-tighter shadow-sm">CRITICAL</span></div>
                            ...
                            <div className="flex items-center text-primary group-hover:translate-x-1 transition-transform"><span className="material-symbols-outlined text-xl">chevron_right</span></div>
                         </div>
                      ))}
                   </div>
                </div>
          ...
          <div className="bg-surface-dark p-6 rounded-3xl border border-white/5 flex flex-col gap-6 shadow-2xl mb-20 relative overflow-hidden">
             ...
             <div className="flex items-center gap-2"><span className="material-symbols-outlined text-primary text-sm">verified_user</span><h3 className="font-black uppercase tracking-[0.2em] text-[10px] italic">Final Dossier Certification</h3></div>
      ...
      {!isSubmitting && (
        <div className="fixed bottom-0 w-full max-w-md bg-background-dark/95 backdrop-blur-xl border-t border-white/5 p-5 pb-10 z-50"><button onClick={handleFinalize} disabled={!sigData} className={`w-full h-16 rounded-2xl font-black uppercase tracking-[0.2em] text-sm flex items-center justify-center gap-3 active:scale-95 transition-all shadow-2xl ${sigData ? 'bg-primary text-white shadow-primary/30' : 'bg-white/5 text-white/20 some-other-class border border-white/5'}`}><span>Lock & Commit Report</span><span className="material-symbols-outlined">verified</span></button></div>
      )}
    </div>
  );
};
...
const SignaturePad: React.FC<{ onSign: (data: string) => void; placeholder: string }> = ({ onSign, placeholder }) => {
  ...
  return (
    <div className="relative w-full h-36 bg-background-dark/80 rounded-2xl border border-white/10 overflow-hidden cursor-crosshair group shadow-inner">{isEmpty && (<div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none opacity-20 text-center px-4"><span className="material-symbols-outlined text-4xl animate-pulse">edit_square</span><span className="text-[8px] font-black uppercase tracking-[0.2em] mt-2 leading-relaxed">{placeholder}</span></div>)}<canvas ref={canvasRef} width={400} height={144} onMouseDown={startDrawing} onMouseMove={draw} onMouseUp={stopDrawing} onMouseLeave={stopDrawing} onTouchStart={startDrawing} onTouchMove={draw} onTouchEnd={stopDrawing} className="w-full h-full touch-none" />{!isEmpty && <button onClick={() => { const c = canvasRef.current; c?.getContext('2d')?.clearRect(0,0,c.width,c.height); setIsEmpty(true); onSign(''); }} className="absolute top-2 right-2 p-2 bg-white/5 hover:bg-primary/20 rounded-xl text-[7px] font-black uppercase text-text-muted hover:text-white transition-all">Clear</button>}</div>
  );
};
